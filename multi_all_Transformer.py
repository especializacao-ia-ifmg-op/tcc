...
def run_model(dataset_file_name, result_file_name, sufix, n_execucoes, n_previsoes):
    #d_model=input_dim, nhead=nhead, num_encoder_layers=num_layers
...
for dataset in os.listdir(bases_path):
    if dataset.endswith('.csv'):
        ...
        print(f'[multi_all_Transformer.py]: base = {dataset_path}\n')
        print(f'Running...')
        
        start = time.time()
        run_model(dataset_file_name=str(dataset_path), result_file_name='./resultados/m_all_results_Transformer_'+str(n_execucoes)+'_'+str(n_previsoes)+'_'+str(dataset), sufix=' (Rs, u2, Tmax, Tmin, RH, pr, ETo)', n_execucoes=n_execucoes, n_previsoes=n_previsoes)
        stop = time.time()

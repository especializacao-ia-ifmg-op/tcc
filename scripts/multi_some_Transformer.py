...
for dataset in os.listdir(caminho_bases):
    if dataset.endswith('.csv'):
        ...
        print(f'[multi_some_Transformer.py]: base = {caminho_dataset}\n')
        print(f'Running...')
        
        start = time.time()
        run_model(dataset_file_name=str(caminho_dataset), result_file_name='./resultados/m_some_results_Transformer_'+str(n_execucoes)+'_'+str(n_previsoes)+'_'+str(dataset), sufix=' (Rs, u2, Tmax, RH, ETo)', n_execucoes=n_execucoes, n_previsoes=n_previsoes)
        stop = time.time()

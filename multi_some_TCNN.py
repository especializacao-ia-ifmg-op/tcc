...
for dataset in os.listdir(bases_path):
    if dataset.endswith('.csv'):
        ...
        print(f'[multi_some_CNN1.py]: base = {dataset_path}\n')
        print(f'Running...')
        
        start = time.time()
        run_model(dataset_file_name=str(dataset_path), result_file_name='./resultados/m_some_results_TCNN_'+str(n_execucoes)+'_'+str(n_previsoes)+'_'+str(dataset), sufix=' (Rs, u2, Tmax, RH, ETo)', star=star_CNN1, n_var =n_var, n_execucoes=n_execucoes, n_previsoes=n_previsoes)
        stop = time.time()
